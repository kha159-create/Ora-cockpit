...
async function getToken(baseUrl: string, tenantId: string, clientId: string, clientSecret: string) {
  ...
  if (!res.ok) throw new Error(`token_error_${res.status}: ${(await res.text()).slice(0, 300)}`);
...
async function fetchAllRows(url: string, token: string) {
  ...
  while (next) {
    ...
    if (!res.ok) throw new Error(`d365_fetch_${res.status}: ${(await res.text()).slice(0, 300)}`);
    const json = await res.json();
...
// BeginDateTime is UTC. POS local time = UTC+5 (verified against POS screenshots).
const POS_OFFSET = 5;
...
function getLocalHourFromBeginDateTime(beginDateTimeStr: string | undefined | null): number {
  ...
  return (d.getUTCHours() + POS_OFFSET) % 24;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const toExclusive = `${nextDay(to)}T00:00:00Z`;
    ...
    const txUrl =
      ...
      `?$select=OperatingUnitNumber,TransactionDate,BeginDateTime,PaymentAmount,TransactionNumber` +
      `&$filter=${txFilter}`;
    ...
    for (const r of rows) {
      ...
      const txDate = txDateRaw.substring(0, 10);
      if (!sid || !txDate || txDate.length !== 10) continue;

      // Daily total keyed by TransactionDate (matches POS business date)
      const dateKey = `${txDate}|${sid}`;
      dailySales.set(dateKey, (dailySales.get(dateKey) || 0) + val);
      if (!dailyTrans.has(dateKey)) dailyTrans.set(dateKey, new Set());
      if (tx) dailyTrans.get(dateKey)!.add(tx);

      // Hourly: SAME TransactionDate + local hour from BeginDateTime+3h
      // This guarantees sum(hourly) == daily total for each date+store
      const localHour = getLocalHourFromBeginDateTime(r.BeginDateTime);
      const hourKey = `${txDate}|${sid}|${localHour}`;
      hourlySales.set(hourKey, (hourlySales.get(hourKey) || 0) + val);
      if (!hourlyTrans.has(hourKey)) hourlyTrans.set(hourKey, new Set());
    ...
    const distinctHours = new Set(salesHourly.map((r) => Number(r[2]))).size;

    return res.status(200).json({
      metadata: {
        source: 'd365-direct',
        time_field: 'BeginDateTime',
        offset: '+5h (UTC to POS local, verified against POS)',
        from,
        to,
