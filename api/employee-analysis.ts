...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const systemPrompt = [
      ...
      'ركز على: جودة المبيعات، متوسط الفاتورة مقابل الفرع، بيع اللحاف، توزيع شرائح سعر اللحاف، ربط اللباد، موديلات اللباد 5/10/15 سم، ربط المخدة على أساس أن لكل لحاف يجب بيع مخدتين، الاعتماد على العروض، وضع الهدف الحالي، وضع المطلوب اليومي، واتساق الأداء بين الفترات.',
      'ميّز بوضوح بين البيع المنظم والبيع غير المتوازن، وبين استخدام العروض بشكل جيد والاعتماد الزائد على العروض.',
      ...
      'لكل موظف مهم، حاول تحديد: مصدر القوة الحقيقي، نقطة الضعف الأساسية، هل البيع منظم أم غير متوازن، ومن أين يأتي الأداء، ثم توصية إدارية مباشرة.',
    ].join(' ');
