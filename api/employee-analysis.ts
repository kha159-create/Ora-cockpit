...
function sanitizeRows(rows: any[]) {
  return rows.slice(0, 180).map((row) => ({
    ...
    padKingPriceFocus: String(row?.padKingPriceFocus || '').trim(),
    padFullPriceFocus: String(row?.padFullPriceFocus || '').trim(),
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const systemPrompt = [
      ...
      'ركز على: جودة المبيعات، متوسط الفاتورة مقابل الفرع، بيع اللحاف، توزيع شرائح سعر اللحاف، ربط اللباد، موديلات اللباد 5/10/15 سم، توزيع اللباد السعري داخل كينغ وداخل فل/كوين/توين، ربط المخدة على أساس أن لكل لحاف يجب بيع مخدتين، الاعتماد على العروض، وضع الهدف الحالي، وضع المطلوب اليومي، واتساق الأداء بين الفترات.',
      'استخدم أيضاً ملخص مزيج المنتجات لتحديد هل المبيعات متنوعة ومنظمة أم مركزة في فئة واحدة أو معتمدة على منتجات ثانوية.',
      'ميّز بوضوح بين البيع المنظم والبيع غير المتوازن، وبين استخدام العروض بشكل جيد والاعتماد الزائد على العروض، وبين التنوع الصحي والتنويع غير المفيد.',
